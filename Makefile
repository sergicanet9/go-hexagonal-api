...
goose-create:
	...
	goose -dir infrastructure/postgres/migrations/ create $${name:-<name>} sql
goose-up:
	go install github.com/pressly/goose/v3/cmd/goose@v3.5.0
	@read -p "Connection string (format: host=XX port=XX dbname=XX user=XX password=XX sslmode=XX): " connection_string; \
	goose -dir infrastructure/postgres/migrations/ -table "public.goose_db_version" postgres "$${connection_string:-<connection_string>} " up
